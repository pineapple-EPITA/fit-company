...
def create_subscription():
    try:
        ...
        return jsonify(subscription.model_dump(mode='json')), 201
...
def process_payment(subscription_id):
    try:
        ...
        return jsonify(payment.model_dump(mode='json')), 201
    except ValidationError as e:
...
def get_subscription(subscription_id):
    try:
        ...
        if subscription:
            return jsonify(subscription.model_dump(mode='json')), 200
        return jsonify({"error": "Subscription not found"}), 404
    except Exception as e:
        return jsonify({"error": "Error retrieving subscription", "details": str(e)}), 500

@billing_bp.route('/users/<string:user_email>/subscriptions', methods=['GET'])
def get_user_subscriptions(user_email):
    try:
        subscriptions = BillingService.get_user_subscriptions(user_email)
        return jsonify([sub.model_dump(mode='json') for sub in subscriptions]), 200
    except Exception as e:
